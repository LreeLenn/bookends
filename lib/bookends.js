...
function getColumns(spec) {
  ...
  if (spec.hydration) {
    ...
    columns = getAggregation(spec).columns(spec);
  }
...
function getSubModel(Model, relationName) {
  var relation = getRelation(Model, relationName);
  return relation && relation.target;
}

function getChildColumns(Model, specs) {
  return _.map(specs, function(spec) {
    var relation = getRelation(Model, spec.relation);
    return relation && relation.foreignKey;
  });
}

function getRelations(hydration, Model, prefix) {
  ...
  return _.reduce(relationSpecs, function(relation, relationSpec) {
    ...
    if (requestedColumns.length) {
      relation[prefix + relationSpec.relation] = function(qb) {
        ...
        var ChildModel = getSubModel(Model, relationSpec.relation);
        var childColumns = getChildColumns(ChildModel, subRelationSpecs);
    ...
    _.each(subRelationSpecs, function(subRelationSpec) {
      var SubModel = getSubModel(Model, subRelationSpec.relation);
      var subRelations = getRelations([subRelationSpec], SubModel, relationSpec.relation);
...
function aggregate(records, spec) {
  return getAggregation(spec).aggregate(records, spec);
}
...
function resolveAggregations(record, Model, hydration) {
  ...
  _.each(subHydrations, function(subHydration) {
    if (_.isArray(record[subHydration.relation]) && getParentColumn(Model, subHydration.relation)) {
      var SubModel = getSubModel(Model, subHydration.relation);
      record[subHydration.relation] = _.map(record[subHydration.relation], function(subRecord) {
        return resolveAggregations(subRecord, SubModel, subHydration.hydration);
      });
    }
  });
...
Bookends.prototype = {
  // not actually using or supporting where yet, so temp removing it
  hydrate: function(Model, hydration) { //where, hydration) {

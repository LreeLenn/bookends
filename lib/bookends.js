...
var builtInAggregators = require('./aggregators');
var cleanRecord = require('./clean-record');
...
Bookends.prototype = {
  _getAggregation: function getAggregation(spec) {
    var repo = (spec.aggregation.custom && this.aggregations) || builtInAggregators;
    return repo[spec.aggregation.method];
  },
  ...
  _cleanUp: function cleanUp(record, Model, hydration) {
    ...
    // this is important, we are only considering hydration based relations
    // aggregations don't get cleaned up, it's up to them to clean themselves
    var relationSpecs = _.filter(hydration, 'hydration');

    record = cleanRecord(record, hydration);

    _.each(relationSpecs, function(relationSpec) {
      var parentColumn = this._getParentColumn(Model, relationSpec.relation);
      var RelatedModel = this._getRelatedModel(Model, relationSpec.relation);
      ...
      if (parentColumn && _.isArray(record[relationSpec.relation])) {
        var childRecords = _.map(record[relationSpec.relation], function(relatedChild) {
          return this._cleanUp(relatedChild, RelatedModel, relationSpec.hydration);
        }, this);
        ...
        record[relationSpec.relation] = this._cleanUp(record[relationSpec.relation], RelatedModel, relationSpec.hydration);
      }
    }, this);
  ...
  hydrate: function hydrate(Model, options, hydration) {
    function buildFinalResult(result) {
      var finalRecords =  _.map(result.models, function(record) {
        record = this._resolveAggregations(record.toJSON(), Model, hydration);
        return this._cleanUp(record, Model, hydration);
      }, this);
